...
#         initial_lr = config.hyperparameters.optimizer.params.lr
#         sorted_epochs = sorted(lr_policy.keys())

#         def lr_lambda(epoch):
#             nonlocal initial_lr
#             new_lr = initial_lr  # Default to initial_lr
...
#             if config.verbose:
#                 logger.info(f"Epoch {adjusted_epoch}: Adjusted learning rate to {new_lr}")
#             return new_lr / initial_lr

#         scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_lambda)
#         # Print the scheduler's current learning rate
#         print('Current learning rate from scheduler:', scheduler.get_last_lr()[0])
#         return scheduler
#     return None
...
def train(model, train_x, train_y, valid_x, valid_y, config, device):
    ...
    loss_function = getattr(nn, hyperparams.loss_function)()
...
def evaluate(name, x, y, model, config, device, wandb=None, epoch=None):
    ...
    loss_fn = getattr(nn, config.hyperparameters.loss_function)(reduction='none')
